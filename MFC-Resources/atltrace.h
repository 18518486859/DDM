...
private:
			...
			_In_z_ LPCWSTR pwszMessage)
	{
		...
		{
			if ((strLen = swprintf_s(wszBuf, TraceBufferSize, L"%s - ", pwszCategoryName)) == -1)
			{
				return;
			}
		}
		...
		{
			if ((strLen = swprintf_s(wszBuf, TraceBufferSize, L"%u - ", dwCategory)) == -1)
			{
				return;
			}
		}

		if ((strLen = swprintf_s(wszBuf + strLen, TraceBufferSize - strLen, L"%s", pwszMessage)) == -1)
		{
			return;
		}
...
public:
	...
	static void __cdecl CTrace::TraceV(
		...
		_In_ va_list args)
	{
		char szBuf[TraceBufferSize] = {'\0'};

		if (_vsnprintf_s(szBuf, TraceBufferSize, TraceBufferSize - 1, pszFmt, args) == -1)
		{
			return;
		}
		...
		_In_ va_list args)
	{
		...
		if (_vsnwprintf_s(wszBuf, TraceBufferSize, TraceBufferSize - 1, pwszFmt, args) == -1)
		{
			return;
		}
